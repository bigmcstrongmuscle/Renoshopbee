...
function eshop_woocommerce_scripts() {
	wp_enqueue_style( 'eshop-woocommerce-style', get_template_directory_uri() . '/woocommerce.css' );
...
if ( ! function_exists( 'eshop_woocommerce_product_columns_wrapper' ) ) {
	/**
	 * Product columns wrapper.
	 *
	 * @return  void
	 */
	function eshop_woocommerce_product_columns_wrapper() {
		$columns = eshop_woocommerce_loop_columns();
		echo '<div class="columns-' . absint( $columns ) . '">';
	}
};
add_action( 'woocommerce_before_shop_loop', 'eshop_woocommerce_product_columns_wrapper', 40 );

if ( ! function_exists( 'eshop_woocommerce_product_columns_wrapper_close' ) ) {
	/**
	 * Product columns wrapper close.
	 *
	 * @return  void
	 */
	function eshop_woocommerce_product_columns_wrapper_close() {
		echo '</div>';
	};
};
add_action( 'woocommerce_after_shop_loop', 'eshop_woocommerce_product_columns_wrapper_close', 40 );
...
add_action( 'woocommerce_before_main_content', 'eshop_woocommerce_wrapper_before' );
...
if ( ! function_exists( 'eshop_woocommerce_cart_link' ) ) {
	...
	function eshop_woocommerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'eshop' ); ?>">
			<?php
